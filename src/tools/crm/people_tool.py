...
class PeopleTool(BaseTool):
    ...
    async def _search_people(self, params: PeopleSearchParams) -> Dict[str, Any]:
        ...
        try:
            ...
            # Format people data for better table display (same as _list_people)
            formatted_people = []
            for person in people:
                ...
                company_names = []
                if person.metaData:
                    for meta in person.metaData:
                        if meta.company and meta.company.name:
                            company_names.append(meta.company.name)
                company_name = ", ".join(company_names) if company_names else None
                
                # Build formatted person dict
                person_dict = {
                    "id": person.id,
                    "full_name": full_name,
                    ...
                    # Include nested data for potential use
                    "email": person.email,
    ...
    async def _list_people(self, limit: int, offset: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Format people data for better table display
            formatted_people = []
            for person in people:
                # Merge first_name and last_name into full_name
                first_name = person.firstName or ""
