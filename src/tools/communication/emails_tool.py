...
from src.infrastructure.database.dynamodb_client import dynamodb_client
...
class EmailSearchParams(BaseModel):
    ...
    person_email: Optional[str] = None  # NEW: lookup person by email
    company_id: Optional[str] = None
    company_name: Optional[str] = None
    company_email: Optional[str] = None  # NEW: lookup company by email
    ...
    keyword: Optional[str] = None  # NEW: keyword search in subject/body


class EmailTool(BaseTool):
    """Tool for email operations using DynamoDB"""

    # Direction mapping: API format -> DynamoDB storage format
    DIRECTION_MAP = {
        'INBOUND': 'received',
        'OUTBOUND': 'sent',
        'inbound': 'received',
    ...
    # Reverse mapping: DynamoDB -> API format
    ...
    def _normalize_direction(self, direction: Optional[str]) -> Optional[str]:
        """
        Normalize direction from API format (INBOUND/OUTBOUND) to DynamoDB format (received/sent).
    ...
    async def _list_emails_by_company_name(
        ...
        try:
            ...
            return {
                ...
                "resolved_contact_info": resolved_contact_info
            }
    ...
    async def _find_person_by_email(self, email_address: str):
        """
        Find person by email address (case-insensitive)
        """
        try:
            client = await prisma_client.get_client()
            ...
            person = await client.people.find_first(
                where={
                    'email': {
                        'some': {
                            'value': {'equals': email_address, 'mode': 'insensitive'}
                        }
                    },
                    'workspaceId': self.workspace_id
                }
            ...
            self.logger.error(f"Error finding person by email '{email_address}': {e}")
            return None

    async def _find_company_by_email(self, email_address: str):
        """
        ...
        try:
            ...
            self.logger.error(f"Error finding company by email '{email_address}': {e}")
            return None
    ...
    async def _get_company_emails(
        ...
        try:
            ...
            if direction:
                ...
                query_params['ExpressionAttributeValues'][':direction'] = normalized_direction
    ...
    def _apply_keyword_filter(self, emails: List[Dict[str, Any]], keyword: str) -> List[Dict[str, Any]]:
        """
        Filter emails by keyword in subject or body with relevance scoring.
        Based on TypeScript searchEmails implementation.

        Relevance Scoring:
        - Exact subject match: +20 points
        - Subject contains keyword: +10 points
        - Body contains keyword: +5 points

        Returns emails sorted by relevance score (highest first)
        """
        if not keyword or not emails:
            return emails
        ...
        for email in emails:
            ...
            # Only include emails with matches
            if score > 0:
                email['relevance_score'] = score
                matching_emails.append(email)

        # Sort by relevance score (highest first)
        matching_emails.sort(key=lambda e: e.get('relevance_score', 0), reverse=True)
    ...
    async def _search_emails(self, params: EmailSearchParams) -> Dict[str, Any]:
        ...
        Can search by person/company name, email, or ID with additional filters.
        ...
        try:
            ...
            if params.company_name and not params.company_id:
                ...
                if company:
                    ...
                    resolved_contact_info = {
                        "name": company.name,
                        "search_query": params.company_name,
                        "type": "company"
                    }
                else:
                    return {
            ...
            # NEW: Search by company email
            if params.company_email and not params.company_id:
                company = await self._find_company_by_email(params.company_email)
                if company:
                    params.company_id = company.id
                    resolved_contact_info = {
                        "name": company.name,
                        "search_query": params.company_email,
                        "type": "company"
                    }
                else:
                    return {
                        ...
                        "has_more": False,
                        "next_token": None,
                        "error": f"Company with email '{params.company_email}' not found"
                    }
            ...
            if params.person_name and not params.person_id:
                ...
                if person:
                    ...
                    resolved_contact_info = {
                        "name": f"{person.firstName} {person.lastName or ''}".strip(),
                        "search_query": params.person_name,
                        "type": "person"
                    }
                else:
                    return {
            ...
            # NEW: Search by person email
            if params.person_email and not params.person_id:
                person = await self._find_person_by_email(params.person_email)
                if person:
                    params.person_id = person.id
                    resolved_contact_info = {
                        "name": f"{person.firstName} {person.lastName or ''}".strip(),
                        "search_query": params.person_email,
                        "type": "person"
                    }
                else:
                    return {
                        ...
                        "has_more": False,
                        "next_token": None,
                        "error": f"Person with email '{params.person_email}' not found"
                    }
            ...
            if params.direction:
                normalized_direction = self._normalize_direction(params.direction)
                filter_expressions.append('direction = :direction')
                expression_values[':direction'] = normalized_direction
            ...
            # NEW: Apply keyword search with relevance scoring (in-memory filtering)
            if params.keyword:
                filtered_emails = self._apply_keyword_filter(filtered_emails, params.keyword)
            ...
            # Add resolved contact info if available
            if resolved_contact_info:
                result["resolved_contact_info"] = resolved_contact_info
    ...
    async def _get_email_analytics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get emails by direction (using DynamoDB storage format)
            sent_response = table.scan(
                ...
                ExpressionAttributeValues={
                    ...
                    ':direction': self._normalize_direction('OUTBOUND')  # 'sent'
                },
                Select='COUNT'
            )
            ...
            received_response = table.scan(
                ...
                ExpressionAttributeValues={
                    ':pk_prefix': f"WORKSPACE#{self.workspace_id}#",
                    ':direction': self._normalize_direction('INBOUND')  # 'received'
                },
                Select='COUNT'
            )
    ...
    def _map_dynamo_item_to_email(self, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if source == 'MANUAL_EVENT':
            ...
            dynamo_direction = item.get('direction', 'received')
            api_direction = self.DIRECTION_REVERSE_MAP.get(dynamo_direction, 'INBOUND')
        ...
        # Convert DynamoDB direction to API format
        dynamo_direction = item.get('direction', 'received')
        api_direction = self.DIRECTION_REVERSE_MAP.get(dynamo_direction, 'INBOUND')
        ...
        return {
            ...
            'from': item.get('from', ''),
            'to': to_field,
            'direction': api_direction,  # Return API format (INBOUND/OUTBOUND)
            'body': body_text,
            'htmlBody': raw_html,
