...
def get_read_tools() -> list:
    ...
    @tool
    async def resolve_object_type(object_type_name: str) -> str:
        """Resolve a natural language object type name (e.g. 'flower', 'Invoice') to an objectDefinition. Call this first before any custom object operation. Returns: id, name, namePlural, slug of the matched definition."""
        context = get_tool_context()
        client = context.get_client()
        ...
        query GetObjectDefinitions($workspaceId: ID!) {
            getObjectDefinitions(workspaceId: $workspaceId) {
                id name namePlural slug description isEnabled
            }
        }
        """
        ...
        try:
            ...
            if not definitions:
                return f"No custom object types found in this workspace."

            normalized = []
            for d in definitions:
                if not d.get("isEnabled"):
                    continue
                normalized.append({**d, "name": d.get("name", "")})

            if not normalized:
                return f"No enabled custom object types found in this workspace."

            matches = fuzzy_match_entities(object_type_name, normalized, name_key="name", threshold=50.0, limit=5)

            if not matches:
                plural_normalized = [{**d, "name": d.get("namePlural", "")} for d in normalized]
                matches = fuzzy_match_entities(object_type_name, plural_normalized, name_key="name", threshold=50.0, limit=5)
                if matches:
                    matches = [(next(d for d in normalized if d["id"] == m[0]["id"]), score) for m, score in matches]

            if not matches:
                slug_normalized = [{**d, "name": d.get("slug", "")} for d in normalized]
                matches = fuzzy_match_entities(object_type_name, slug_normalized, name_key="name", threshold=50.0, limit=5)
                if matches:
                    matches = [(next(d for d in normalized if d["id"] == m[0]["id"]), score) for m, score in matches]

            if not matches:
                type_list = ", ".join(d.get("name", "") for d in normalized)
                return f"Could not find object type matching '{object_type_name}'. Available types: {type_list}"
            ...
            if len(matches) == 1 or score_gap >= 15:
                defn = matches[0][0]
                return (
                    f"RESOLVED: Object type '{defn['name']}' (plural: {defn.get('namePlural', defn['name']+'s')}) "
                    f"has objectDefinitionId: {defn['id']} (slug: {defn.get('slug', '')})"
                )
            else:
                response = request_entity_selection(
                    entity_type="object_type",
                    name_query=object_type_name,
                    matches=matches,
                    name_key="name",
                )
                if response.confirmed and response.selected_id:
                    selected = next((m[0] for m in matches if m[0]["id"] == response.selected_id), None)
                    selected_name = selected["name"] if selected else "Unknown"
                    return f"RESOLVED: Object type '{selected_name}' has objectDefinitionId: {response.selected_id}"
                else:
                    feedback = f" User feedback: {response.feedback}" if response.feedback else ""
                    return f"User cancelled object type selection.{feedback}"
            ...
            if 'Interrupt' in type(e).__name__:
                raise
            logger.error(f"Error resolving object type: {e}")
            return f"Error resolving object type: {str(e)}"
        finally:
            await client.close()
