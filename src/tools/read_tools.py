...
# ---------------------------------------------------------------------------
# Composite Tools — bundle resolve + fetch to eliminate LLM round-trips
# ---------------------------------------------------------------------------

@tool
async def search_and_get_company(name: str) -> str:
    """Find a company by name and return its full details in one step.

    Combines name resolution (with fuzzy matching) and detail fetching.
    Use this instead of calling resolve_company_name then get_company_by_id.

    Args:
        name: Company name (handles typos, partial names, case differences)

    Returns:
        Full company details, or asks user to select if ambiguous
    """
    context = get_tool_context()
    client = context.get_client()
    ...
    query GetOneCompany($companyId: ID!) {
        getOneCompany(companyId: $companyId) {
            id name description imageUrl avatarUrl createdAt updatedAt
            emails { id value type isPrimary }
            phoneNumbers { id value type isPrimary }
            addresses { id value type isPrimary }
            urls { id label value isPrimary }
            peopleMetaData { people { id firstName lastName jobTitle } }
            groupCompanies { group { id name emoji } }
        }
    }
    """
    ...
    try:
        # Step 1: Resolve name
        result = await client.query(search_query, {
            "workspaceId": context.workspace_id, "limit": 20, "search": name,
        })
        companies = result.get("getWorkspaceCompany", {}).get("data", [])
            ...
            if len(name_parts) > 1:
                ...
                for part in name_parts:
                    ...
                    r = await client.query(search_query, {
                        "workspaceId": context.workspace_id, "limit": 20, "search": part,
                    })
                    for hit in r.get("getWorkspaceCompany", {}).get("data", []):
                        if hit.get("id") and hit["id"] not in combined:
                            combined[hit["id"]] = hit
                companies = list(combined.values())

        if not companies:
            return f"Could not find any company matching '{name}'."

        matches = fuzzy_match_entities(name, companies, name_key="name", threshold=50.0, limit=5)
        if not matches:
            return f"Could not find any company matching '{name}'."

        # Determine the resolved company ID
        top_score = matches[0][1]
        second_score = matches[1][1] if len(matches) > 1 else 0
        if len(matches) == 1 or (top_score - second_score) >= 15:
            company_id = matches[0][0]["id"]
        else:
            response = request_entity_selection(
                entity_type="company", name_query=name, matches=matches, name_key="name",
            )
            if response.confirmed and response.selected_id:
                company_id = response.selected_id
            else:
                feedback = f" User feedback: {response.feedback}" if response.feedback else ""
                return f"User cancelled company selection.{feedback}"

        # Step 2: Fetch full details
        detail_result = await client.query(detail_query, {"companyId": company_id})
        company = detail_result.get("getOneCompany")
        if not company:
            return f"Company resolved but details not found (ID: {company_id})."

        lines = [format_company(company)]
        urls = company.get("urls", [])
        if urls:
            url_parts = [f"{u.get('label', 'URL')}: {u.get('value', '')}" for u in urls]
            lines.append(f"  URLs: {', '.join(url_parts)}")
        addresses = company.get("addresses", [])
        if addresses:
            addr_strs = [a.get("value", "") for a in addresses if a.get("value")]
            if addr_strs:
        ...
        people_meta = company.get("peopleMetaData", [])
        if people_meta:
            names = [f"{p['people']['firstName']} {p['people'].get('lastName','')}".strip()
                     for p in people_meta if p.get("people")]
            if names:
                lines.append(f"  Associated People: {', '.join(names)}")
        groups = company.get("groupCompanies", [])
        if groups:
            gnames = [f"{g['group'].get('emoji','')} {g['group']['name']}".strip()
                      for g in groups if g.get("group")]
            if gnames:
                lines.append(f"  Groups: {', '.join(gnames)}")

        return "\n".join(lines)

    except Exception as e:
        logger.error(f"Error in search_and_get_company: {e}")
        return f"Error finding company: {str(e)}"
    finally:
        await client.close()


@tool
async def search_and_get_person(name: str) -> str:
    """Find a person by name and return their full details in one step.

    Combines name resolution (with fuzzy matching) and detail fetching.
    Use this instead of calling resolve_person_name then get_person_by_id.

    Args:
        name: Person's name (handles typos, partial names, reordering)

    Returns:
        Full person details, or asks user to select if ambiguous
    """
    context = get_tool_context()
    client = context.get_client()
    ...
    query GetWorkspacePeople($workspaceId: ID!, $limit: Int, $search: String) {
        getWorkspacePeople(workspaceId: $workspaceId, limit: $limit, search: $search) {
            data { id firstName lastName jobTitle }
        }
    }
    """
    ...
    query GetPerson($peopleId: ID!) {
        getPerson(peopleId: $peopleId) {
            id firstName lastName jobTitle description dateOfBirth gender
            avatarUrl imageUrl createdAt updatedAt
            emails { id value type isPrimary }
            phoneNumbers { id value type isPrimary }
            addresses { id type value isPrimary }
            urls { id label value isPrimary }
            companyMetaData { company { id name imageUrl } }
            groupPeople { group { id name emoji } }
        }
    }
    """

    def normalize(p):
        return {**p, "name": f"{p.get('firstName','')} {p.get('lastName','')}".strip()}
    ...
    try:
        ...
        people = [normalize(p) for p in result.get("getWorkspacePeople", {}).get("data", [])]
        ...
        if not people:
            ...
            if len(name_parts) > 1:
                ...
                for part in name_parts:
                    ...
                    for hit in r.get("getWorkspacePeople", {}).get("data", []):
                        if hit.get("id") and hit["id"] not in combined:
                            combined[hit["id"]] = normalize(hit)
                people = list(combined.values())

        if not people:
            return f"Could not find any person matching '{name}'."
        ...
        if len(matches) == 1 or (top_score - second_score) >= 15:
            ...
            response = request_entity_selection(
                entity_type="person", name_query=name, matches=matches, name_key="name",
            )
            if response.confirmed and response.selected_id:
                person_id = response.selected_id
            else:
                feedback = f" User feedback: {response.feedback}" if response.feedback else ""
                return f"User cancelled person selection.{feedback}"

        # Step 2: Fetch full details
        detail_result = await client.query(detail_query, {"peopleId": person_id})
        person = detail_result.get("getPerson")
        if not person:
            return f"Person resolved but details not found (ID: {person_id})."
        ...
        logger.error(f"Error in search_and_get_person: {e}")
...
def get_pipeline_tools() -> list:
    """Tools for pipeline/status operations and column management."""
    return [
...
def get_email_tools() -> list:
    """Tools for email interactions, templates, and drafting."""
    return [
...
def get_read_tools() -> list:
    """Get all READ tools (Core + Search + Details + Pipeline + Email)."""
    return (
        get_core_tools()
        + get_search_tools()
        + get_details_tools()
